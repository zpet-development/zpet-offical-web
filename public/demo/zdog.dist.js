...
if (this.texture != false){this.pattern = ctx.createPattern(this.texture, "repeat");}
  ctx.fillStyle = (this.texture!= false) ? this.pattern : this.getRenderColor();
  ctx.beginPath();
...
if (this.borderType>=0){
  ...
  var radius = lineWidth/2;
  ctx.lineWidth = 1;
  if (this.borderType==0){ctx.arc( point.x, point.y, radius, 0.5*Math.PI, 1.5*Math.PI );}else if (this.borderType==1){ctx.arc( point.x, point.y, radius, 1.5*Math.PI, 0.5*Math.PI );}else if (this.borderType==2){ctx.arc( point.x, point.y, radius, 0*Math.PI, 2*Math.PI );}
  ctx.stroke();
}
}else{
ctx.translate(-30,-30);
ctx.drawImage(this.image, point.x, point.y);
ctx.translate(30,30);
}
